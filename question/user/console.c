...
void* load( char* x ) {
  ...
  else if( 0 == strcmp( x, "d" ) ) {
    return &main_Dining;
  }
  else if( 0 == strcmp( x, "test_write" ) ) {
    return &test_write_file;
  }
  else if( 0 == strcmp( x, "test_read" ) ) {
    return &test_read_file;
  }
  else if( 0 == strcmp( x, "test_wc" ) ) {
    return &test_wc;
  }
  else{
...
void main_console() {
  ...
  while( 1 ) {
    ...
    else if( 0 == strcmp( p, "cs" ) ) { 
      int alg_index = atoi( strtok( NULL, " " ) );  
      change_sche(alg_index);
    } 
    else if( 0 == strcmp( p, "state" ) ) {
      state();
    } 
    else if( 0 == strcmp( p, "disk" ) ) {
      char num[10];
      itoa(num, disk_block_num());
      puts("disk block number: ", 19);
      puts(num, 10);
      puts("\n", 1);
      char len[5];
      itoa(len, disk_block_len());
      puts("disk block length: ", 19);
      puts(len, 5);
      puts("\n", 1);
    } 
    else if( 0 == strcmp( p, "ls" ) ) {
      char* filenames = (char*) list_file();
      puts( filenames, 16 * 15 );
    }
    else if( 0 == strcmp( p, "rm" ) ) {
      char* filename = strtok( NULL, " " ) ;
      int r = remove_file(filename);
      if (r < 0) puts( "wrong file name\n", 16 );
    }
    else {
